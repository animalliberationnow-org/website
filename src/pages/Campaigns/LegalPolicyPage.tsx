...
const LegalPolicyPage = () => {
  return (
    <div className="bg-primary min-h-screen">
      <Hero
        title="Legal & Policy"
        subtitle="Lasting change requires systemic overhaul. We push for policy changes that protect animals from institutional exploitation."
        ...
        <div className="max-w-6xl mx-auto relative z-10 px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-32">
            <div className="glass-panel p-10 group text-center hover:-translate-y-2">
              <h3 className="text-4xl font-black mb-6 text-text-main group-hover:text-accent transition-colors drop-shadow-glow-white mt-1">PROPERTY</h3>
              <p className="text-text-muted font-medium text-sm uppercase tracking-widest leading-relaxed">Animals are legally classified as property, denying them fundamental rights.</p>
            </div>
            <div className="glass-panel p-10 group text-center hover:-translate-y-2">
              <h3 className="text-4xl font-black mb-6 text-text-main group-hover:text-accent transition-colors drop-shadow-glow-white mt-1">EXEMPTIONS</h3>
              <p className="text-text-muted font-medium text-sm uppercase tracking-widest leading-relaxed">Standard agricultural practices are often exempt from cruelty laws.</p>
            </div>
            <div className="glass-panel p-10 group text-center hover:-translate-y-2">
              <h3 className="text-4xl font-black mb-6 text-text-main group-hover:text-accent transition-colors drop-shadow-glow-white mt-1">SUBSIDIES</h3>
              <p className="text-text-muted font-medium text-sm uppercase tracking-widest leading-relaxed">Taxpayer money often subsidizes animal exploitation industries.</p>
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section dark={false}>
        ...
        <div className="glass-panel p-12 max-w-4xl mx-auto text-center shadow-neon">
          <p className="text-xl text-text-muted leading-relaxed font-medium">
            While street activism changes hearts, policy change alters the landscape. We engage with local and regional bodies to advocate for plant-based initiatives, defunding of factory farms, and stronger animal welfare protections as stepping stones toward total liberation.
          </p>
        </div>
      </Section>
          ...
          <div className="glass-panel p-1 border-accent/30 shadow-neon">
            <div className="bg-primary-light p-12 flex flex-col md:flex-row items-center justify-between gap-12 rounded-[2.5rem]">
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                <Link to="/support" className="btn btn-outline text-sm">
                  Sign Petitions
                </Link>
                <Link to="/support" className="btn btn-primary text-sm flex items-center justify-center">
                  Lobby With Us <HiArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </div>
            </div>
          </div>
